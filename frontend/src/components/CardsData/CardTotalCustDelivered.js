...
import { FaTruck } from "react-icons/fa"; // Truck icon for delivered orders
import { fetchCustomerDelivery } from "../../api/CustomerDeliveryApi";

const CardTotalCustDelivered = () => {
  ...
  useEffect(() => {
    const fetchDeliveredOrders = async () => {
      try {
        ...
        setDeliveredCount(
          data.filter((order) => order.OUTBOUND_DEL_STATUS === "Delivered").length
        );
      } catch (error) {
...
export default CardTotalCustDelivered;
